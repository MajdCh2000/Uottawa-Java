	...
	public static int getOptimalNumberOfSpots(int hourlyRate) {
		...
		while(!found) {
			...
			for (int i = 0; i < NUM_RUNS; i++) {
				...
				Simulator simulation = new Simulator(lot, hourlyRate, 24 * 3600);
				long start = System.currentTimeMillis();
				simulation.simulate();
				long end = System.currentTimeMillis();
				int thisQueue = simulation.getIncomingQueueSize();
				System.out.println("Simulation run " + i + " (" + (end - start) + "ms); Queue length at the end of simulation run: " + thisQueue);
				averageQueue += thisQueue;
			}

			averageQueue = averageQueue / NUM_RUNS;

			if (averageQueue <= THRESHOLD) {
				found = true;
			}
		}
