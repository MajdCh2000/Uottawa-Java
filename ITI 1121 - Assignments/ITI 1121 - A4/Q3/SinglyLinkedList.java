...
public class SinglyLinkedList<E> implements List<E>, Iterable<E> {
    ...
    private int count(Node<E> p, E fromElement, E toElement, int i) {
        if (p == null) {
            return i;
        } else if (i == 0 && p.value.equals(toElement) && p.value.equals(fromElement)){
            return 1;
        } else if (i == 0 && p.value.equals(fromElement)) {
            return count(p.next, fromElement, toElement, 1);
        } else if (p.value.equals(toElement) && i > 0) {
            return i + 1;
        } else if (i == 0) {
            return count(p.next, fromElement, toElement, i);
        }
        return count(p.next, fromElement, toElement, i + 1);
    }
