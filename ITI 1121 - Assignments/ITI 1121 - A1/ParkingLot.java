	...
	private void populateFromFile(String strFilename) throws Exception {
		...
		while (scanner.hasNext()) {
			...
			if (before) {
				...
				if (str.length() > 0) {
					char c = str.charAt(2);
					CarType ca = Util.getCarTypeByLabel(Character.toString(c));
					Car car = new Car(ca, str.substring(3, 5));
					
					if (canParkAt(Character.getNumericValue(str.charAt(0)), Character.getNumericValue(str.charAt(1)), car)) {
						this.park(Character.getNumericValue(str.charAt(0)), Character.getNumericValue(str.charAt(1)), car);
					} else {
						System.out.println("Car " + Util.getLabelByCarType(car.getType()) + "(" + car.getPlateNum() + ") cannot be parked at (" + str.charAt(0) + "," + str.charAt(1) + ")");
					}
				}
			}
		}
